...
- Architecture:
    - `hdhomerun_mcp/api/`: Modular folder for target service client wrappers.
    - `hdhomerun_mcp/mcp/`: Modular folder for action-routed dynamic MCP tool tags.
    - `hdhomerun_mcp/mcp_server.py`: Main MCP server entry point and tool registration.
    - `hdhomerun_mcp/agent_server.py`: Pydantic AI agent definition and logic.
...
- MCP Entry Point → `hdhomerun_mcp/mcp_server.py`
- Agent Entry Point → `hdhomerun_mcp/agent_server.py`
- Source Code → `hdhomerun_mcp/`
- API client mixins → `hdhomerun_mcp/api/`
- MCP tool modules → `hdhomerun_mcp/mcp/`
- Tests → `tests/`
